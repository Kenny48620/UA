...
int testcase_main()
{
    ...
    USLOSS_Console("EXPECTATION: testcase_main() creates child XXp1(), priority 3.  It creates its own child, XXp3(), priority 5.  It stores the pid of the XXp3() child into a global, and then blocks on join().  testcase_main() wakes up and creates another child, XXp2(), priority 4.  This calls zap() on the pid stored in the global variable, meaning that *two* processes are now blocked on the same XXp3().  XXp3() and testcase_main() race; XXp3() will call dumpProcesses() and die, while testcase_main() will join().  When XXp3() dies, XXp1() and XXp2() will both be awoken but XXp1() will run first.\n");
...
int XXp2(char *arg)
{
    ...
    zap_rc = zap(pid_e);
    USLOSS_Console("XXp2(): after zap'ing child with pid_e, zap_rc = %d\n", zap_rc);
